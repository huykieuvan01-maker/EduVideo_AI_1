...
export default function App() {
  ...
  const runStepWithFallback = async (
    ...
    for (const model of modelsToTry) {
      updateState(model, 'loading');
      try {
        const response = await fetch('/api/educore/generate', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'x-api-key': userApiKey,
          },
          body: JSON.stringify({ prompt, mode, step: stepNumber, model }),
        });
        ...
        if (response.ok) {
          ...
          console.warn(`Model ${model} failed for Step ${stepNumber}:`, lastError);
        }
      } catch (err: any) {
        lastError = err.message || 'Network error';
        console.warn(`Model ${model} threw error for Step ${stepNumber}:`, lastError);
      }
    }
